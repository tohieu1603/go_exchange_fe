...
type BottomTab = 'open_orders' | 'order_history' | 'trade_history' | 'funds';
        ...
        {ticker ? (
          <>
            ...
            <div className="hidden md:flex items-center gap-5 text-[11px]">
              ...
              <div>
                <div className="text-text-muted">24h Volume({base})</div>
                <div className="text-text-primary font-mono tabular-nums">{volFmt(ticker.volume24h)}</div>
              </div>
              <div>
                <div className="text-text-muted">24h Volume({quote})</div>
                <div className="text-text-primary font-mono tabular-nums">{volFmt(ticker.volume24h * ticker.price)}</div>
              </div>
            </div>
          </>
        ) : (
          <span className="text-[13px] text-text-muted">{base}/{quote}</span>
        )}
      </div>
      ...
      <div className="flex flex-col md:hidden flex-1">
        ...
        <div className="bg-bg-secondary" style={{ height: '260px', overflow: 'hidden' }}>
          {mobilePanel === 'orderbook'
            ? <OrderBook pair={pair} onPriceClick={setClickedPrice} />
            : <TradeHistory pair={pair} />}
        </div>

        {/* Order Form */}
        <div className="bg-bg-secondary border-t border-border">
          <OrderForm pair={pair} initialPrice={clickedPrice} />
        </div>
      ...
      <div className="hidden md:flex flex-col flex-1 min-h-0 gap-px bg-bg-primary" style={{ height: 'calc(100vh - 88px)' }}>
        ...
        <div className="bg-bg-secondary shrink-0 flex flex-col overflow-hidden" style={{ height: '200px' }}>
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            ...
            {bottomTab === 'order_history' && <OrderHistory pair={pair} />}
            {bottomTab === 'trade_history' && <TradeHistory pair={pair} />}
            {bottomTab === 'funds' && (
              <div className="flex items-center justify-center h-full text-text-muted text-[11px]">
                Log in to view your funds
              </div>
            )}
          </div>
        </div>
